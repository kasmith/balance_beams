...
get_resampled_winners_all = function(dat, sym_cost, mass_cost, phys_cost, N=100, elim_dist=T) {
  if (elim_dist) {
    udat = dat %>% filter(!grepl("d", OrderType))
  } else {
    udat = dat
  }
  # NOTE: Assumes weight/dist rule costs are identical
  calc = value_of_computation(udat, sym_cost, mass_cost, mass_cost, phys_cost) %>%
...
find_reasonable_strategies_wdist = function(calculated, thresh = 10) {
  return(calculated %>%
           mutate(strat_msp_smp = (msp + smp) >= thresh,
                  strat_dsp_sdp = (dsp + sdp) >= thresh,
                  strat_sstar_mdp = (smdp + msdp + mdsp) >= thresh,
                  strat_sstar_dmp = (sdmp + dsmp + dmsp) >= thresh,
                  strat_sp = sp >= thresh,
                  strat_p_only = p >= thresh,
                  strat_ms_sm_s_m = (m+s+ms+sm) >= thresh,
                  strat_nop = (m+s+d+ms+md+sm+sd+dm+ds+msd+mds+smd+sdm+dms+dsm) >= thresh,
                  strat_guess = guessing >= thresh)
  )
}
...
multi_plot_phys_strats_wdist = function(phys_costs = seq(.03, .27, by=.03), reasonable_thresh = 10, data=rat_dat) {
  nphyscosts = length(phys_costs)
  all_params = data.frame(phys_cost = rep(phys_costs, each=55)) %>%
    mutate(sym_cost = phys_cost*rep(sequence(1:10)/10, times=nphyscosts),
           mass_cost = phys_cost*rep(rep(1:10, times=1:10)/10, times=nphyscosts))
...
indstrat_plot = function(dat, title=NULL) {
  #crp = colorRampPalette(c('blue', 'red'))
  #myscale = scale_fill_gradientn(colors=crp(50), limits=c(0,1))
  myscale = scale_fill_gradientn(colors=c(colorRampPalette(c('blue','red'))(25), colorRampPalette(c('red','yellow'))(25)), limits=c(0,1))
  plt = ggplot(dat, aes(x=mass_cost, y=sym_cost, fill=PropWin, z=PropWin)) +
    geom_raster() +
    facet_wrap(~phys_cost, scales='free') +
    theme_bw() +
    xlab("Weight Rule Cost") +
